...
description: >-
  ...
  root-cause debugging, use power-automate-mcp, power-automate-debug, or the
  server monitor-flow bundle. Requires FlowStudio for Teams or MCP Pro+.
---

# Power Automate Monitoring with FlowStudio MCP

> **⚠️ Not available in the free local server (v1).** This skill relies on the
> `store_*` cached-store tools. The free, local Power Automate MCP server in this
> repo (`server/`) ships the **16 live tools only** — it does **not** include any
> `store_*` tool. These monitoring workflows therefore require the hosted
> FlowStudio MCP Pro+ service (or another server that implements the `store_*`
> tool family). If you are on the free local server, use `power-automate-mcp`,
> `power-automate-debug`, or `power-automate-build` instead, which work entirely
> with the live tools.

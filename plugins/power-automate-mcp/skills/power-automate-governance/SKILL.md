...
> `store_*` tool. These governance workflows therefore require the hosted
> FlowStudio MCP Pro+ service (or another server that implements the `store_*`
> tool family). If you are on the free local server, the live tools
> (`power-automate-mcp`, `power-automate-build`, `power-automate-debug`) cover
> reading, building, debugging, and state control — but not cached-store metadata.
